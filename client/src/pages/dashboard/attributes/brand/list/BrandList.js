...
import BrandTagFiltered from './BrandTagFiltered';
import BrandNewEditFormDialog from '../new-edit/BrandNewEditFormDialog';
// utils

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: 'code', label: 'Mã thương hiệu', align: 'left' },
...
export default function BrandList() {
  ...
  const {
    ...
    setData,
    setTotalPages,
  } = useFetch(ADMIN_API.brand.all);

  const onFinish = (data, type, id) => {
    if (type === "create") {
      onOpenSuccessNotify('Thêm mới thương hiệu thành công!')
    ...
    if (type === "update") {
      onOpenSuccessNotify('Cập nhật thương hiệu thành công!')
      setData(data?.brands);
      setTotalPages(data?.totalPages);
      setCurrentObj(data?.brands.find((item) => item.id === id));
      setOpen(false);
    }

    if (type === "delete") {
      onOpenSuccessNotify('Xóa thương hiệu thành công!')
      setData(data?.brands);
      setTotalPages(data?.totalPages);
      setStatusCounts(data?.statusCounts);
    }
  ...
  const handleDelete = (id) => {
    ...
    showConfirm("Xác nhận xóa thương hiệu?", () => remove(ADMIN_API.brand.delete, body, (res) => onFinish(res, "delete")));
  ...
  const handleSave = (data, type) => {
    if (type === 'create') {
      ...
      showConfirm("Xác nhận thêm mới thương hiệu?", () => post(ADMIN_API.brand.post, body, (res) => onFinish(res, type)));
    }
    ...
    if (type === 'update') {
      ...
      showConfirm("Xác nhận cập nhật thương hiệu?", () => put(ADMIN_API.brand.put, body, (res) => onFinish(res, type, body.id)));
    }
  }
  ...
  const handleUpdateStatus = (data) => {
    ...
    put(ADMIN_API.brand.putStatus, body, (res) => onFinishUpdateStatus(res));
  }
  ...
  return (
    <>
      <Page title="Quản lý thương hiệu - Danh sách thương hiệu">
        <Container >
          <HeaderBreadcrumbs
            heading="Danh sách thương hiệu"
            links={[
              { name: 'Quản lý thương hiệu', href: PATH_DASHBOARD.attribute.brand.list },
      ...
      <BrandNewEditFormDialog
        open={open}
        onClose={handleClose}
        onSave={handleSave}
        currentBrand={currentObj}
        isEdit={isEdit}
      />
    </>
  );
